...
namespace {
  ...
  response::ping::handler make_response_wrapper(size_t size, response::ping::handler cb, ipingpong::pong_handler pong_reqester)
  {
    ...
    std::get<0>(p) = std::make_shared< std::atomic_size_t >( size );
...
void pingpong::stat_( time_t ms, size_t pingc, size_t pongc )
{
  ...
  if ( !i.empty() )
  {
    ...
    PINGPONG_LOG_MESSAGE( 
      "Totals "<< pingc << "<->" << pongc
      << std::setprecision(3) << std::fixed
      << " pings " << _stat_count << " time " <<  tm_ns/1000000.0
      << " rate " << ns2rate(tm_ns, _stat_count) << " rate" << k << " " << ns2rate(tm_ns, _stat_count)*k 
    )
    _show_time = now;
    _stat_time = std::chrono::high_resolution_clock::now();
    _stat_count = 0;
